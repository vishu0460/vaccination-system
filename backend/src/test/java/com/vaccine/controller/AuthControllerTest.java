...
RegisterRequest req = new RegisterRequest("test@example.com", "Test User", "Password123", 25, "+1234567890");
        when(authService.register(any(), any())).thenReturn(new ApiMessage("Registered successfully"));
    ...
    void login_WithValidCredentials_ShouldReturnAuthResponse() throws Exception {
        LoginRequest loginReq = new LoginRequest("user@example.com", "password", null);
        AuthResponse response = new AuthResponse("accessToken", "refreshToken", "Bearer", 900L, "user@example.com", "USER");
    ...
    void login_WithInvalidCredentials_ShouldReturnBadRequest() throws Exception {
        LoginRequest loginReq = new LoginRequest("user@example.com", "wrongpassword", null);
        when(authService.login(any(), any())).thenThrow(new org.springframework.security.authentication.BadCredentialsException("Invalid credentials"));
    ...
    void refresh_WithValidToken_ShouldReturnAuthResponse() throws Exception {
        ...
        AuthResponse response = new AuthResponse("newAccessToken", "newRefreshToken", "Bearer", 900L, "user@example.com", "USER");
    ...
    void forgotPassword_WithValidEmail_ShouldReturnSuccess() throws Exception {
        ...
        when(authService.forgotPassword(any())).thenReturn(new ApiMessage("Password reset token sent"));
    ...
    void resetPassword_WithValidToken_ShouldReturnSuccess() throws Exception {
        ...
        when(authService.resetPassword(any())).thenReturn(new ApiMessage("Password reset successful"));
